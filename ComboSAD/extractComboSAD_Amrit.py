import sys, os
sys.path.insert(0, '/nfs/turbo/brownpriori/Libraries/lib/python2.7/site-packages')
import numpy as np
...
def removeNoiseBias(signal, windowSize, stepSize):
    ...
    outSignal = signal
    outSignal = np.pad(outSignal, int(windowSize/2), 'median', stat_length= windowSize/2)
...
def extractComboSAD(origAudio, Fs, minSpeechSec=0.3, minSilenceSec=0.7, maxSpeechSec=30.0):
    ...
    feats = np.full((int(nFrames), nFeats), np.nan)
    ...
    #return [origAudio[x:y] for x,y in zip(starts, stops)]